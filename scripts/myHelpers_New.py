...
def findFixedEta(myTOB,myTOBVector):
    ...
    if cellEM2max == 0:   FixedEta = myTOBVector.Eta() - (0.025 + 0.0125)
    elif cellEM2max == 1: FixedEta = myTOBVector.Eta() - 0.0125
    elif cellEM2max == 2: FixedEta = myTOBVector.Eta() + 0.0125
    elif cellEM2max == 3: FixedEta = myTOBVector.Eta() + (0.025 + 0.0125)
    return FixedEta
...
def clusDens(myTOB):
    myCells = createCellLists(myTOB)
    # treat Had as EM3 to reduce fluctuations
    layerVolume = [147.0*147.0*10.0, 147.0*36.8*90.0, 147.0*36.8*336.0, 147.0*147.0*42.0, 147.0*147.0*42.0]
...
def weightDist(myVector):
    ...
    for i in range(0, 36):
        ...
        runningSum += math.sqrt(pow(abs(index1-1)*0.1, 2) + pow(abs(index2-5.5) * 0.025, 2)) * myVector[i]
...
def bigCluster6(myTOB):
    ...
    midPhiEM1 =[
    allCells[1][13]+allCells[1][14]+allCells[1][15]+allCells[1][16]+allCells[1][17]+allCells[1][18],
    allCells[1][15]+allCells[1][16]+allCells[1][17]+allCells[1][18]+allCells[1][19]+allCells[1][20],
    allCells[1][17]+allCells[1][18]+allCells[1][19]+allCells[1][20]+allCells[1][21]+allCells[1][22]
    ]
    midPhiEM2 =[
    allCells[2][13]+allCells[2][14]+allCells[2][15]+allCells[2][16]+allCells[2][17]+allCells[2][18],
    allCells[2][15]+allCells[2][16]+allCells[2][17]+allCells[2][18]+allCells[2][19]+allCells[2][20],
    allCells[2][17]+allCells[2][18]+allCells[2][19]+allCells[2][20]+allCells[2][21]+allCells[2][22]
    ]

    #for phi=0 and phi=1 i'll just loop over the two blocks sum energy (6,7 and 8,9 and 10,11 and 12,13)
    upPhiEM1 = [allCells[1][1]+allCells[1][2],allCells[1][3]+allCells[1][4],allCells[1][5]+allCells[1][6],allCells[1][7]+allCells[1][8],allCells[1][9]+allCells[1][10]]
    upPhiEM2 = [allCells[2][1]+allCells[2][2],allCells[2][3]+allCells[2][4],allCells[2][5]+allCells[2][6],allCells[2][7]+allCells[2][8],allCells[2][9]+allCells[2][10]]
    lowPhiEM1 = [allCells[1][1+24]+allCells[1][2+24],allCells[1][3+24]+allCells[1][4+24],allCells[1][5+24]+allCells[1][6+24],allCells[1][7+24]+allCells[1][8+24],allCells[1][9+24]+allCells[1][10+24]]
    lowPhiEM2 = [allCells[2][1+24]+allCells[2][2+24],allCells[2][3+24]+allCells[2][4+24],allCells[2][5+24]+allCells[2][6+24],allCells[2][7+24]+allCells[2][8+24],allCells[2][9+24]+allCells[2][10+24]]

    energyEM1EM2_mid    = [midPhiEM1[0]+midPhiEM2[0],midPhiEM1[1]+midPhiEM2[1],midPhiEM1[2]+midPhiEM2[2]]
    energyEM1EM2_low    = [lowPhiEM1[0]+lowPhiEM2[0],lowPhiEM1[1]+lowPhiEM2[1],lowPhiEM1[2]+lowPhiEM2[2],lowPhiEM1[3]+lowPhiEM2[3],lowPhiEM1[4]+lowPhiEM2[4]]
    energyEM1EM2_up     = [upPhiEM1[0]+upPhiEM2[0],upPhiEM1[1]+upPhiEM2[1],upPhiEM1[2]+upPhiEM2[2],upPhiEM1[3]+upPhiEM2[3],upPhiEM1[4]+upPhiEM2[4]]
...
def bigClusterCal(myTOB):
    ...
    energyPSEM3HAD = 0.5*energyPS[0]+0.5*energyPS[1]+0.5*energyPS[2]+1.9*energyEM3[0]+1.9*energyEM3[1]+energyHAD[0]+energyHAD[1]+energyHAD[2]
...
def bigCluster6Cal(myTOB, SFHad):
    ...
    energyEM1EM2_mid    = [midPhiEM1[0]+1.0*midPhiEM2[0],midPhiEM1[1]+1.0*midPhiEM2[1],midPhiEM1[2]+1.0*midPhiEM2[2]]
    energyEM1EM2_low    = [lowPhiEM1[0]+1.0*lowPhiEM2[0],lowPhiEM1[1]+1.0*lowPhiEM2[1],lowPhiEM1[2]+1.0*lowPhiEM2[2],lowPhiEM1[3]+1.0*lowPhiEM2[3],lowPhiEM1[4]+1.0*lowPhiEM2[4]]
    energyEM1EM2_up     = [upPhiEM1[0]+1.0*upPhiEM2[0],upPhiEM1[1]+1.0*upPhiEM2[1],upPhiEM1[2]+1.0*upPhiEM2[2],upPhiEM1[3]+1.0*upPhiEM2[3],upPhiEM1[4]+1.0*upPhiEM2[4]]
    ...
    energyPSEM3HAD = 0.5*energyPS[0]+0.5*energyPS[1]+0.5*energyPS[2]+ 1.9*energyEM3[0]+1.9*energyEM3[1]+1.9*energyHAD[0]+1*energyHAD[1]+1*energyHAD[2]
...
def bigClusIso(length,myTOB):
    ...
    try: Iso_EM1EM2EM3HAD = (energyEM1EM2+E_HAD+E_EM3)/(E_EM1_de+E_EM2_de+E_EM3_de+E_HAD_de)
    except: Iso_EM1EM2EM3HAD = 0.0
    try: Iso_EM1EM2HAD = (energyEM1EM2+E_HAD)/(E_EM1_de+E_EM2_de+E_HAD_de)
    except: Iso_EM1EM2HAD = 0.0
    try: Iso_EM1EM2 = energyEM1EM2/(E_EM1_de+E_EM2_de)
    except: Iso_EM1EM2 = 0.0
    try: Iso_all = allE/(E_PS_de+E_EM1_de+E_EM2_de+E_EM3_de+E_HAD_de)
    except: Iso_all = 0.0
    return [Iso_all, Iso_EM1EM2, Iso_EM1EM2HAD, Iso_EM1EM2EM3HAD, allE, E_PS_de+E_EM1_de+E_EM2_de+E_EM3_de+E_HAD_de, energyEM1EM2, E_EM1_de+E_EM2_de, energyEM1EM2+E_HAD, E_EM1_de+E_EM2_de+E_HAD_de, energyEM1EM2+E_HAD+E_EM3, E_EM1_de+E_EM2_de+E_EM3_de+E_HAD_de]
...
def ApplyIsoTLV1(S, pt):
    ...
    elif S > (0.0009 * pt + 0.69): doCut = True
    #print S, pt, 0.0023 * pt, 0.0023 * pt + 0.76
    return doCut
...
def ApplyIsoTLV2(S, pt):
    ...
    elif S > (0.0013 * pt + 0.69): doCut = True
    #print S, pt, 0.0023 * pt, 0.0023 * pt + 0.76
    return doCut
...
def ApplyIsoTLV3(S, pt):
    ...
    #    if S > 0.85: doCut = True
    #if PT > 60000.0: doCut = True
    elif S > (0.0011 * pt + 0.69): doCut = True
...
#    if((EM2allCell[EM2seedindex-12]+EM1allCell[EM2seedindex-12]) > (EM2allCell[EM2seedindex+12]+EM1allCell[EM2seedindex+12])):
    ...
    for i in range(5):
        EM12_midRow += EM2allCell[EM2seedindex-2+i] + EM1allCell[EM2seedindex-2+i]
        if(doUp):
            EM12_extraPhi += (EM2allCell[EM2seedindex-2+i-12] + EM1allCell[EM2seedindex-2+i-12])
        else:
            EM12_extraPhi += (EM1allCell[EM2seedindex-2+i+12] + EM2allCell[EM2seedindex-2+i+12])
...
def findPeak(myTOB, layer):
    ...
    return [max1, max2, ratio, distance, myLayerSum(myList), max1index, max2index] #output changed
...
#    print("etarange %i"%etaRange)
    for i in range(3):#phi loop
        for j in range(etaRange):#eta loop
            if(layer==2 or layer==5 or layer==6): eCell = myTOB.getEnergy(layer, j+1, i+1)
            else: eCell = myTOB.getEnergy(layer, j+4, i+1)
            if ( eCell> maxE ):
                maxE = eCell
                maxEindex = [j,i]
            else:
    ...
    for layer in [2, 3, 4, 5, 6]:
        for i in range(3):#phi loop
            for j in range(3):#eta loop
                if(layer==2 or layer==5 or layer==6): CT[i][j] += myTOB.getEnergy(layer, j+1, i+1)
                ...
                else: CT[i][j] += (myTOB.getEnergy(layer, 4*j+4, i+1)+myTOB.getEnergy(layer, 4*j+5, i+1)+myTOB.getEnergy(layer, 4*j+6, i+1)+myTOB.getEnergy(layer, 4*j+7, i+1))
...
def findAllMax(myTOB):
    ...
    maxAll = [[eta_Max_PS,phi_Max_PS],[eta_Max_EM1,phi_Max_EM1],[eta_Max_EM2,phi_Max_EM2],[eta_Max_EM3,phi_Max_EM3],[eta_Max_HAD,phi_Max_HAD]]
...
# I'll seed with max in EM2(L=4)
    ...
    EM2_EIndex = findMax2(4,myTOB)[1] #(eta_max,phi_max)=(EM2_EIndex[0],EM2_EIndex[1]) remember to add 4 to eta for layers EM2 and EM1 (and always 1 to phi)

    E_env_EM2 = 0
    for o in range(3):
        for p in range(3):
            if (myTOB.getEnergy(4, EM2_EIndex[0]-1+4+o, EM2_EIndex[1]-1+1+p)>0):
                E_env_EM2 +=myTOB.getEnergy(4, EM2_EIndex[0]-6+4+o,EM2_EIndex[1]-1+1+p)>0 #not sure about -6
            #if (myTOB.getEnergy(4, EM2_EIndex[0]-1+4+o, EM2_EIndex[1]-1+1+p)>0):
...
#now I go to the previous layer (EM1) which has the same granularity and check  the hottest cell for all the cells sorrounding the aligned one (including that one)    
    ...
    for o in range(3):
        for p in range(3):
            if maxEM1 < myTOB.getEnergy(3, EM2_EIndex[0]+4-1+o, EM2_EIndex[1]+1-1+p):
                maxEM1 = myTOB.getEnergy(3, EM2_EIndex[0]+4-1+o, EM2_EIndex[1]+1-1+p)
                maxEM1_I = [EM2_EIndex[0]+4-1+o,EM2_EIndex[1]-1+1+p]
    E_env_EM1 = 0
    for o in range(3):
        for p in range(3):
            if (myTOB.getEnergy(3, maxEM1_I[0]-1+o, maxEM1_I[1]-1+p)>0): 
                E_env_EM1 += myTOB.getEnergy(3, maxEM1_I[0]-6+o, maxEM1_I[1]-1+p)
            #if (myTOB.getEnergy(3, maxEM1_I[0]-1+o, maxEM1_I[1]-1+p)>0):
...
#now go to the previous layer (the first one, which is PS)
    ...
    for o in range(3):
        for p in range(3):
            if maxPS < myTOB.getEnergy(2, PS_EIndex_projection[0]+1-1+o, PS_EIndex_projection[1]+1-1+p) and myTOB.getEnergy(1, PS_EIndex_projection[0]+1-1+o, PS_EIndex_projection[1]+1-1+p)>0:
                maxPS = myTOB.getEnergy(2, PS_EIndex_projection[0]+1-1+o, PS_EIndex_projection[1]+1-1+p)
                maxPS_I = [PS_EIndex_projection[0]+1-1+o,PS_EIndex_projection[1]-1+1+p]
...
#now I go to the next layer (EM3), which is 3x3, and first look for the cell with activity which has the smallest distance from the alignement to EM2
    ...
    for o in range(3):
        for p in range(3):
            if maxEM3 < myTOB.getEnergy(5, EM3_EIndex_projection[0]+1-1+o, EM3_EIndex_projection[1]+1-1+p) and myTOB.getEnergy(5, EM3_EIndex_projection[0]+1-1+o, EM3_EIndex_projection[1]+1-1+p)>0:
                maxEM3 = myTOB.getEnergy(5, EM3_EIndex_projection[0]+1-1+o, EM3_EIndex_projection[1]+1-1+p)
                maxEM3_I = [EM3_EIndex_projection[0]+1-1+o,EM3_EIndex_projection[1]-1+1+p]
...
def hotNext75(myTOB, factor):
    ...
    if nextE>factor*maxEL2_PS: sumE += nextE
    ...
    if nextE>factor*maxEL3_EM1: sumE += nextE
    ...
    if nextE>factor*maxEL4_EM2: sumE += nextE
    ...
    if nextE>factor*maxEL5_EM3: sumE += nextE
    ...
    if nextE>factor*maxEL6_HAD: sumE += nextE
...
def hotNextNf25(myTOB, factor, Nfactor):
    ...
    if nextNE>Nfactor*maxEL2_PS: sumE += nextNE
    ...
    if nextNE>Nfactor*maxEL3_EM1: sumE += nextNE
    ...
    if nextNE>Nfactor*maxEL4_EM2: sumE += nextNE
    ...
    if nextNE>Nfactor*maxEL5_EM3: sumE += nextNE
    ...
    if nextNE>Nfactor*maxEL6_HAD: sumE += nextNE
...
class histHolder:
    ...
    def __init__(self, listOfCriteria):
        for myCrit in listOfCriteria:
            self.toPlots[myCrit] =  ROOT.TProfile("TurnOn"+myCrit,"TurnOn"+myCrit, 400, 0, 200)
            self.toPlots[myCrit].SetXTitle("true #tau vis. p_{T}")
            self.toPlots[myCrit].SetYTitle("Efficiency")

            self.toEtaPlots[myCrit] =  ROOT.TProfile("TurnOnEta"+myCrit,"TurnOnEta"+myCrit, 100, 1, 2)
            ...
            self.ntobPlots[myCrit] = ROOT.TH1D("nTOB"+myCrit, "nTOB"+myCrit, 400, 0, 200)
