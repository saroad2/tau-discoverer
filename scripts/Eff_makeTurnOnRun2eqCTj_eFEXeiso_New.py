...
MinBiasRateFile = ROOT.TFile("output/turnOnCurve_minbiasCTj_Timing_New_eFEXeiso_Eff.root")
...
myCritsForThre = ["nTOBTAU12", "nTOBTAU20","nTOBTAU12R3","nTOBTAU20R3","nTOBTAU12jR3","nTOBTAU20jR3"]
    ...
    for trueTau in truthTaus:
        ...
        if abs(nearestTau.Eta()) > 2.47 or nearestTau.Pt()<10000 or smallestDR > 0.2: 
            ...
            for jTau in run3jTaus:
                if abs(jTau.Eta()) > 2.47: continue
                myDR = nearestTau.TLV.DeltaR(jTau.TLV)
                if myDR < newDR:
                    newDR = myDR
                    jcandTauPt = jTau.Pt()
                    jcandTauIso = jTau.Iso
            if (nearestTau.Pt()>=12000. and nearestTau.Pt()<15000.): 
                n12 += 1
            elif (nearestTau.Pt()>=15000. and nearestTau.Pt()<25000.): 
                n15 += 1
            elif nearestTau.Pt()>=25000.: 
                n25 += 1
            nFull += 1
            ejDR.Fill(newDR) #answer Q1
            if newDR < 10000.0 and abs(nearestTau.Eta())<1.37: #abs(nearestTau.Eta())>1.52 and abs(nearestTau.Eta())<2.47: #abs(nearestTau.Eta())<1.37: # and trueTau.Pt() > 15000 and abs(trueTau.Eta())<2.47:
                nDR += 1
                ...
                if (nearestTau.Pt()>=10000. and nearestTau.Pt()<11000.):
                    jiso10.Fill(nearestTau.Iso)
                elif (nearestTau.Pt()>=11000. and nearestTau.Pt()<12000.):
                    jiso11.Fill(nearestTau.Iso)
                elif (nearestTau.Pt()>=12000. and nearestTau.Pt()<13000.):
                    n12j += 1
                    eiso12.Fill(nearestTau.Iso)
                    jiso12.Fill(nearestTau.Iso)
                    if (jcandTauIso/jcandTauPt) < 0.3: myPlots.fillTO('TAU12R3', trueTau.Pt(), nearestTau.Pt() > pTthreTAU12R3*1000 )
                    else: myPlots.fillTO('TAU12R3', trueTau.Pt(), 0)
                elif (nearestTau.Pt()>=13000. and nearestTau.Pt()<14000.):
                    jiso13.Fill(nearestTau.Iso)
                elif (nearestTau.Pt()>=14000. and nearestTau.Pt()<15000.):
                    jiso14.Fill(nearestTau.Iso)
                elif (nearestTau.Pt()>=15000. and nearestTau.Pt()<16000.):
                    n15j += 1
                    eiso15.Fill(nearestTau.Iso)
                    jiso15.Fill(nearestTau.Iso)
                    #if nearestTau.Iso > 0.61: 
                    #    ejiso15.Fill(jcandTauIso/jcandTauPt)
                    if (jcandTauIso/jcandTauPt) < 0.32: myPlots.fillTO('TAU12R3', trueTau.Pt(), nearestTau.Pt() > pTthreTAU12R3*1000 )
                    else: myPlots.fillTO('TAU12R3', trueTau.Pt(), 0)
                elif (nearestTau.Pt()>=16000. and nearestTau.Pt()<17000.):
                    jiso16.Fill(nearestTau.Iso)
                elif (nearestTau.Pt()>=17000. and nearestTau.Pt()<18000.):
                    jiso17.Fill(nearestTau.Iso)
                elif (nearestTau.Pt()>=18000. and nearestTau.Pt()<19000.):
                    jiso18.Fill(nearestTau.Iso)
                elif (nearestTau.Pt()>=19000. and nearestTau.Pt()<20000.):
                    jiso19.Fill(nearestTau.Iso)
                elif (nearestTau.Pt()>=20000. and nearestTau.Pt()<21000.):
                    jiso20.Fill(nearestTau.Iso)
                elif (nearestTau.Pt()>=21000. and nearestTau.Pt()<22000.):
                    jiso21.Fill(nearestTau.Iso)
                elif (nearestTau.Pt()>=22000. and nearestTau.Pt()<23000.):
                    jiso22.Fill(nearestTau.Iso)
                elif (nearestTau.Pt()>=23000. and nearestTau.Pt()<24000.):
                    jiso23.Fill(nearestTau.Iso)
                elif (nearestTau.Pt()>=24000. and nearestTau.Pt()<25000.):
                ...
                #    else: myPlots.fillTO('TAU12R3', trueTau.Pt(), 0)
                    ...
                    n25j += 1
                    myPlots.fillTO('TAU12R3', trueTau.Pt(), nearestTau.Pt() > pTthreTAU12R3*1000 )
            else: myPlots.fillTO('TAU12R3', trueTau.Pt(), nearestTau.Pt() > pTthreTAU12R3*1000) # 23/09: used to be 0 but now nearestTau.Pt() > pTthreTAU12R3*1000
                
        # Find nearest, nearest candidate, jFEX
        nearestTau = None
        smallestDR = 50.0
        for candTau in run3jTaus:
            if abs(candTau.Eta()) > 2.47 or candTau.Pt() < 12000: continue
            myDR = trueTau.DeltaR(candTau.TLV)
            if myDR < smallestDR:
                nearestTau = candTau
                smallestDR = myDR
...
# Store output files
myNewFile = ROOT.TFile("/afs/cern.ch/user/b/barak/public/turnOnCurveRun2eqCTj_Timing_New_eFEXeiso1.37.root", "RECREATE")
