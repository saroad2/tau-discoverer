...
for myEvt in myTree:
    ...
    for trueTau in truthTaus:
        ...
        if abs(nearestTau.Eta()) > 2.47 or nearestTau.Pt() < 10000 or smallestDR > 0.2:
            ...
            if (
                newDR < 0.15
                and abs(nearestTau.Eta()) > 1.52
                and abs(nearestTau.Eta()) < 2.47
            ):  # abs(nearestTau.Eta())<1.37: # and trueTau.Pt() > 15000 and abs(trueTau.Eta())<2.47:
                nDR += 1
...
myNewFile = ROOT.TFile(
    "/afs/cern.ch/user/b/barak/public/turnOnCurveRun2eqCTj_Timing_New_eFEXjiso1.52.root",
    "RECREATE",
)
